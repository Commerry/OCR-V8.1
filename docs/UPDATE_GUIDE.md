...
### ตรวจว่าพร้อมใช้งานแล้วจริง

```powershell
cd $env:USERPROFILE\cam-tools
.leet-timesync.ps1 -Verify
```

ตรวจให้ทีละตัว 6 อย่าง และขึ้น **พร้อมใช้งาน** ก็ต่อเมื่อผ่านครบ:

| ตรวจอะไร | ผ่านเมื่อ |
|---|---|
| มี `src/utils/timeSync.js` | ไฟล์อยู่ในเครื่อง |
| สิทธิ์ตั้งเวลา **ใช้ได้จริง** | สั่ง `sudo -n timedatectl` ได้โดยไม่ถามรหัส (ไม่ใช่แค่ไฟล์มีอยู่) |
| โปรแกรมทำงาน | `pm2` มีโปรเซส online |
| นาฬิกา | ต่างจากเครื่องที่สั่งไม่เกิน 60 วินาที |
| timezone | ลงท้าย `+0700` |
| ต่อ Center ได้ | `curl /api/health` ได้ 200 |

พร้อมบอกด้วยว่าเคยซิงก์เวลาไปแล้วกี่ครั้ง และบรรทัด log ล่าสุด

### พิสูจน์ว่าแก้นาฬิกาเองได้จริง

ทดสอบแบบไม่ต้องรอไฟดับ: เลื่อนนาฬิกากล้องถอยหลังโดยตั้งใจ แล้วดูว่ามันดึงกลับมาเอง

```powershell
.leet-timesync.ps1 -TestSelfHeal -Hosts 10.36.181.35
```

สคริปต์จะตั้งเวลาให้ผิดไป 3 ชั่วโมง รอ 75 วินาที (heartbeat 1-2 รอบ) แล้ววัดใหม่
**ผ่าน** = เวลาต่างจากเครื่องที่สั่งไม่เกิน 60 วินาที

ทำทีละตัวก่อน อย่าทำทั้ง fleet พร้อมกัน — ระหว่างทดสอบค่าที่อ่านได้จะถูกบันทึกด้วยเวลาผิดชั่วคราว (เหมือนตอนไฟดับจริง) ลบทิ้งภายหลังได้ด้วยคำสั่งลบแถวอนาคตบน Center
