...
for host in "${HOSTS[@]}"; do
    ...
    OUT=$($SSH_CMD $SSH_OPTS "$SSH_USER@$host" "$REMOTE" 2>&1 | tail -1)
    if echo "$OUT" | grep -qE '^[0-9]{4}-[0-9]{2}-[0-9]{2}'; then
        ok=$((ok + 1))
        BEFORE=$(echo "$OUT" | awk '{print $1" "$2}')
        # how far off was it, in minutes
        DIFF=$(( ( $(date -d "$NOW" +%s) - $(date -d "$BEFORE" +%s 2>/dev/null || echo "$(date -d "$NOW" +%s)") ) / 60 ))
        DIFF=${DIFF#-}
        if [ "$DIFF" -gt 5 ]; then
            drifted=$((drifted + 1))
